...
from app.services.rolling_mean_service import sync_rolling_mean_streaming

router = APIRouter()
TABLE = "maize_predictions_rolling_mean"
...
async def sync_rolling():
    """Compute 5-yr rolling-mean baseline predictions and write to DB."""
    return StreamingResponse(_sse_wrap(sync_rolling_mean_streaming()), media_type="text/event-stream", headers=SSE_HEADERS)
