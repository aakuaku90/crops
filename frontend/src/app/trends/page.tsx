...
export default function SignalsPage() {
  ...
  // Drives the initial-load skeleton; flips when the headline fetch resolves.
  ...
  // ── #7 Yield Trends — top crops by absolute YoY yield change.
  ...
  // ── Trending crop selector — biggest absolute YoY production change.
  // Used to drive the hero chart so the page surfaces the crop with the
  // most newsworthy movement this season.
  const trendingMover = useMemo(() => {
    return [...productionSignals]
      .sort((a, b) => Math.abs(b.yoyPct) - Math.abs(a.yoyPct))[0];
  }, [productionSignals]);

  // Build the data slice CropBalanceChart needs for the trending crop.
  ...
  }, [trendingMover, balanceSignals, producerPriceHistory, population]);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <SignalCard
          ...
          subtitle="Biggest year-over-year change in national production (FAO Crop Production)"
          empty="Awaiting FAO Crop Production data"
          tone="up"
          rows={productionMovers.map((p) => ({
            label: p.crop,
            sub: `${formatTonnes(p.latestValue)} t in ${p.latestYear}`,
        ...
        <SignalCard
          title="Yield Trends"
          subtitle="Productivity change — production ÷ area harvested, year-over-year"
          empty="Awaiting FAO area + production data"
          tone="up"
          rows={yieldMovers.map((y) => ({
            label: y.crop,
            sub: `${y.latestYield.toFixed(2)} t/ha in ${y.latestYear}`,
